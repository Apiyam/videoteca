...
import { Send, CheckCircle, Chat, Search, WhatsApp } from '@mui/icons-material';
...
export default function WhatsAppBot() {
  ...
  useEffect(() => {
    const fetchClientLeadsFunction = async () => {
      setLoading(true);
      const clientLeads = await fetchClientLeads();
      if(clientLeads){
        setClientLeads(clientLeads);
      }
      setLoading(false);
    }
  ...
  useEffect(()=> {
    const fetchMyChat = async ()=>{
      ...
      if(chats){
        setChatMessages(chats.map((chat:any)=>({
          id: chat.id,
  ...
  return (
    <Stack spacing={2}>
      <Card variant="outlined" sx={{ p: 2 }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} sx={{ mt: 2, height: 480 }}>
            ...
            <List sx={{ py: 0 }}>
              {clientLeads.map((chat) => (
                <ListItem
                  ...
                  sx={{
                    cursor: 'pointer',
                    bgcolor: selectedNumber === chat.phone_number ? 'neutral.softBg' : 'transparent',
                    '&:hover': { bgcolor: 'neutral.softBg' },
                    p: 1.5,
                    mb: 1,
                    border: '1px solid',
                    borderColor: selectedNumber === chat.phone_number ? 'primary.plainColor' : 'divider',
                    borderRadius: 'md',
                  }}
                >
                  <Box sx={{ flex: 1, ml: 1 }}>
                    <Typography level="title-sm">{chat.full_name} <small style={{ fontSize: '0.8rem', color: 'gray', paddingLeft: '10px' }}>{formatDate(chat.timestamp, true, true)}</small> </Typography>
              ...
              <>
                ...
                >
                  <Box>
                    ...
                    <a href={`https://wa.me/${selectedChat.phone_number}`} target="_blank" rel="noopener noreferrer">
                      <Button variant="outlined" color="primary">
                        <WhatsApp />
                        Enviar mensaje WhatsApp
                      </Button>
                      </a>
                    <Typography level="body-xs" color="neutral" sx={{position: 'absolute', right: 20, top: 20}}>
                  ...
                  {chatMessages.map((msg, i) => (
                    <Box
                      ...
                      sx={{
                        ...
                        bgcolor: msg.from === 'user' ? 'neutral.softBg' : 'success.softBg',
                        color: 'text.primary',
                        p: 1.2,
                        borderRadius: 'xl',
                        maxWidth: '75%',
