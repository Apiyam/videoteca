  ...
  <Stack spacing={2}>
    ...
    <Card>
      <Box sx={{ mb: 1, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' } }}>
        <Stack>
          <Typography level="title-md">Listado de mis materiales de Burlesqa</Typography>
          <Typography level="body-sm">Estos son los videos y cursos que has comprado.</Typography>
        </Stack>
        <Link href="/escuela/videos-y-cursos">
        <Button sx={{ mt: { xs: 1, sm: 0 } }} color="success" startDecorator={<VideoCameraBack />} size="sm" variant="solid">
        Ver más materiales</Button>
        </Link>
      </Box>
      <Divider />
      ...
      >
          ...
          <tbody>
            ...
            <tr key={videoElement.id}>
              ...
              <td>
                <Link href={`/escuela/videos-y-cursos/${videoElement.id}`}>
