...
export default function MyLeadContacts() {
  ...
  <Stack spacing={2}>
    ...
    <Card>
      <Box sx={{ mb: 1, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' } }}>
        <Stack>
          <Typography level="title-md">Listado de mis contactos</Typography>
          <Typography level="body-sm">Aquí aparecerán los contactos interesados que han contactado con tu negocio.</Typography>
        </Stack>
      </Box>
      <Divider />
    ...
    >
      ...
      >
          ...
          <tbody>
            {contacts?.map((contact) => (
              <tr key={contact.id}>
               ...
               {
                /*
                 <IconButton sx={{ mr: 1 }} color="success" variant="soft" onClick={() => {setContactToEdit(contact); setOpenModal(true)}}>
                  <EditRounded />
                </IconButton>
        ...
        <Typography level="body-md">Aún no hay contactos en tu negocio.</Typography>
      )}
      </Sheet>
        </Stack>
    </Card>
